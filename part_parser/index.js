...
function partSortByName(a, b) {
  var aName = a.name[a.name.length-1];
  var bName = b.name[b.name.length-1];
  if(aName < bName) { return -1; }
  if(aName > bName) { return 1; }
  return 0;
}

// Extract all of the parts from the config files
var allParts = [];
...
// So we are guaranteed to have 'v2' parts show up after 'v1' partSort
// sort the parts first
allParts.sort(partSortByName);
